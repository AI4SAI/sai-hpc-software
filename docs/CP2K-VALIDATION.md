...
The resolver manifest is generated by `controller/cp2k_spack.py`. It pins
...
separate store/buildcache and records the GCC 13.3 external compiler and ISA.
This is intentionally a resolver manifest, not a claim that CP2K is already
Spack-built: cuSOLVERMp still needs an explicit CP2K adapter, and the existing
nvidia ELPA module remains an explicitly CUDA-linked external until CPU/GPU
scientific validation proves its use.
