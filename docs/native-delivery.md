...
当前先验证 **DSPRHBM、16V100**：总入口的 `all` 仅选择这两个分区，并拒绝
显式选择 4V100/8V100V0。DeePMD/LAMMPS、GPUMD 仍是 GPU-only，因此本轮
只调度 16V100。其余分区的编译能力保留在软件分支中，待这两分区的原生运行、
科学结果和速度验收完成后再推广；不能把此前 4V100 的结果当作两分区已通过。
