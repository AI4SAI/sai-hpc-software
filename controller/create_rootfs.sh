...
if [[ -f "$root/.sai-hpc-ci-rootfs-v2" ]]; then exit 0; fi
rm -rf "$root"
mkdir -p "$root"/{bin,etc,dev,proc,sys,tmp,var/tmp,home,root,lib64,usr/bin,opt/devtools}
...
touch "$root/.sai-hpc-ci-rootfs-v2"
