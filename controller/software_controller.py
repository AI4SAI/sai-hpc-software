...
def submit(args):
    ...
    script.write_text(f'''#!/bin/bash\n#SBATCH --job-name={args.software}-{args.run_id}\n#SBATCH --partition={args.partition}\n#SBATCH --qos={args.qos}\n#SBATCH --nodes=1\n#SBATCH --ntasks=1\n{gpu}#SBATCH --time={args.timeout}\n#SBATCH --output={r}/results/slurm-%j.log\nset -eo pipefail\nexport LD_LIBRARY_PATH=\"${{LD_LIBRARY_PATH:-}}\"\nsource /etc/profile.d/lmod.sh\nmodule load apptainer/1.4.4\nset -u\napptainer exec --cleanenv --containall --no-home --writable-tmpfs --bind {r}/source.bundle:/input/source.bundle:ro --bind {r}/results:/output/results:rw --bind /opt/devtools:/opt/devtools:ro --bind /usr:/usr:ro --bind /lib:/lib:ro --bind /lib64:/lib64:ro --bind /opt:/opt:ro --bind /usr:/usr:ro --bind /lib:/lib:ro --bind /lib64:/lib64:ro {args.image} /bin/sh -lc 'export PATH=/opt/devtools/cmake/3.31.6/bin:/usr/bin:/bin; cd /workspace && exec {quoted}'\n'''); script.chmod(0o700)
