...
class WorkflowSourceTests(unittest.TestCase):
    ...
    def test_daily_dispatch_inputs_resolve_all_three_tracks_and_four_registered_partitions(self):
        for software in ("abacus", "cp2k"):
            with self.subTest(software=software):
                rows = self.execute(software, tracks=",".join(contract.TRACKS),
                                    targets=",".join(contract.SOFTWARE[software]["targets"]))
    ...
    def test_dispatch_skips_precisely_absent_channels_but_propagates_network_errors(self):
        for software in ("abacus", "cp2k"):
            ...
            self.assertEqual(self.execute(software, tracks="prerelease,release",
                                         missing=("latest-prerelease", "latest-release")), [])
